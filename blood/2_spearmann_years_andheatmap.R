...
final_resultsyr<-final_resultsyr[, 1:3]
result<-result[, 1:3]
## exclude mean BMI, not interesting. 
result<-subset(result, BMI_measure != 'meanBMI_tdiag')
result<-subset(result, BMI_measure != 'meanBMI_t514')
...
hm_all$BMI_measure<-factor(hm_all$BMI_measure, 
                           levels = rev(c('BMI', '0-2 y', '2-4 y', '4-6 y', '6-8 y', '8-10 y', '10-12 y', 'BMI_5_14earlier')),
                           labels = rev(c('At diagnosis', '0-2y ago', '2-4y ago', '4-6y ago', '6-8y ago', '8-10y ago', '10-12y ago',
                                          '5-14y earlier')))

hm_all$score<-factor(hm_all$score,
                     levels = c("PI_mdson", "PI_mc", "PI_hmn", "PI_do", "PI_mzbach"),
                     labels = c("mBMI-135", "mBMI-1109", "mBMI-400", "mBMI-398", "mBMI-3506"))
hm_all$avg_spearman_coeff
...
ggplot(hm_all, aes(score, BMI_measure)) +                           
  ...
  labs(title = '', x='Methylation-based BMI score', y='Self-reported BMI', fill = "Spearman's coefficient")+
