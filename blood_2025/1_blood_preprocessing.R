...
blood_clin<-merge(base, f, by = 'tn_id') # 5483

blood_clin<-subset(blood_clin, !is.na(blood_clin$timeD))
...
##
blood_clin<-blood_clin[(blood_clin$lab_nummer %in% beta_ILM$tn_id), ] ## 2242
rm(beta_ILM)
...
library(prodlim)
quantile(prodlim(Hist(timey, death_all)~1, data = blood_clin, reverse = T))
summary(as.factor(blood_clin$death_all))
summary(as.factor(blood_clin$death_crc))
summary(as.factor(blood_clin$death_crccp))
